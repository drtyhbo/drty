...
var MySQLConnectionPool = new Class({
	...
	get: function(callback) {
		...
		if (!this.pool.length) {
			if (this.numConnections >= (config.MAX_CONNECTIONS || 15)) {
				...
				var mysqlConn = require("mysql-libmysqlclient").createConnectionSync(config.HOST,
...
exports.MySQL = Class({
	...
	prepareColumn: function(Model, name, column) {
		...
		case 'string':
			if (!column.maxLength) {
				dirty.error(Model.__meta.tableName + "." + name + ": string fields require a 'maxLength' property when using MySQL.")
			}
			__mysql = {
				sqlName: name,
		...
		Model.__mysql = {
			...
			createModelFromRow: function(row, selectRelated) {
				...
				for (var name in columns) {
					...
					values[name] = row[columns[name].__mysql.sqlSelectName];
				}
