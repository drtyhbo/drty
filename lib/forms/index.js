...
var ModelForm = Form.extend({
	save: function(cb) {
		var saveModel = function(model) {
			...
			// copy the data from the form to the model
			for(var name in fields) {
				var column = columns[name];
				if (!column || column._fieldType == 'AutoField') {
					continue;
				}
				// one edge case is file fields. if the file hasn't been uploaded again,
				// use the data that's in the model
				if ((column._fieldType == 'FileField' || column._fieldType == 'ImageField')
			...
			this.clean(function(err) {  
				...
				model.save(function(err, model) {
					if (err) {
						...
						// now reload the model and generate a new form
		...
		// see if the model has already been created by doing a lookup by pk
