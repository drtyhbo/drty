...
var Field = exports.Field = drty.Class.extend({
	...
	validate: function(value) {
		if ((value === undefined || value === '') && this.isRequired()) {
			throw new ValidationError(errorMessages['required'], 'required');
		}                   
	}
});
...
var CharField = exports.CharField = Field.extend({
	...
	validate: function(value) {
		...
		if (this.options.minLength && value.length < this.options.minLength) {
			throw new ValidationError(errorMessages['minLength'], 'minLength');
		}
		if (this.options.maxLength && value.length > this.options.maxLength) {
			throw new ValidationError(errorMessages['maxLength'], 'maxLength');
		}
	}
});
		...
		if (value !== undefined) {
			if (value instanceof Array) {
				for (var i = 0; i < value.length; i++) {
					if (!(value[i] in this.choices)) {
						throw new ValidationError(errorMessages['invalidChoice'], 'invalidChoice');
					}
				}
			} else if (!(value in this.choices)) {
				throw new ValidationError(errorMessages['invalidChoice'], 'invalidChoice');
			}
		}
	}
...
var FloatField = exports.FloatField = Field.extend({
	...
	initialize: function(options) {
		this.parent(drty.utils.merge({
			errorMessages: {'invalid': 'A decimal number is required'}
		}, options));
	},
...
var DecimalField = exports.DecimalField = FloatField.extend({
	...
	validate: function(value) {
		...
		if (value) {
			...
			if (this.options.maxDigits && (whole.length + decimal.length > this.options.maxDigits)) {
				throw new ValidationError(errorMessages['maxDigits'], 'maxDigits');
			}
			if (this.options.decimalPlaces && decimal.length > this.options.decimalPlaces) {
				throw new ValidationError(errorMessages['maxDecimalPlaces'], 'maxDecimalPlaces');
			}
		}
	}
});
