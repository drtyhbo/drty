...
var Field = exports.Field = drty.Class.extend({
	...
	initialize: function(options) {
		...
		this.widget = this.options.widget || new this.defaultWidget();
...
var NumberField = Field.extend({
	...
	initialize: function(options) {
		...
		if (options && options.minValue) {
			this.defaultValidators.push(validators.minValueValidator(options.minValue));
		}
		if (options && options.maxValue) {
			this.defaultValidators.push(validators.maxValueValidator(options.maxValue));
		}
	}	
});
...
var DecimalField = exports.DecimalField = FloatField.extend({
	...
	initialize: function(options) {
		...
		if (options && options.maxDigits) {
			this.defaultValidators.push(validators.maxDigitsValidator(options.maxDigits));
		}
		if (options && options.decimalPlaces) {
			this.defaultValidators.push(validators.decimalPlacesValidator(options.decimalPlaces));
		}
...
var DateTimeField = exports.DateTimeField = Field.extend({
	...
	fromJS: function(value) {
		return drty.utils.strftime(value, this.options.format || this.defaultFormat);
	},
	
	validate: function(value) {
		this.parent(value);
		if (value && !this.stringToDate(value)) {
			throw new ValidationError(this.errorMessageWhenInvalid, 'invalid');
		}
	},
	
	stringToDate: function(str) {
		var inputFormats = this.options.inputFormats ||
...
var JSONField = exports.JSONField = Field.extend({
	...
	toJS: function(value) {
		try {
			...
			throw new ValidationError('JSON is expected', 'invalid');
		}
	},
