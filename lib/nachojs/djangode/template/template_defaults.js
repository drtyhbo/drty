...
var tags = exports.tags = {
    ...
    'for': function (parser, token) {
        ...
        var parts = tags.get_args_from_token(token, { exclude: 2, mustbe: { 2: 'in', 4: 'reversed'} });
    ...
    'ifchanged': function (parser, token) {
        var parts = tags.get_args_from_token(token);
    ...
    'ifequal': function (parser, token) {
        var parts = tags.get_args_from_token(token, { argcount: 2 });
    ...
    'autoescape': function (parser, token) {
        var parts = tags.get_args_from_token(token, { argcount: 1, mustbe: { 1: ['on', 'off'] }});
    ...
    'block': function (parser, token) {
        var parts = tags.get_args_from_token(token, { argcount: 1 });
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.BlockNode(node_list, parts[0]);
    },

    'extends': tags.simple_tag(nodes.ExtendsNode, { argcount: 1 }),

    'firstof': tags.simple_tag(nodes.FirstOfNode),

    'with': function (parser, token) {
        var parts = tags.get_args_from_token(token, { argcount: 3, exclude: 2, mustbe: { 2: 'as' }});
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.WithNode(node_list, parts[0], parts[1], parts[2]);
    },
    
    'now': tags.simple_tag(nodes.NowNode, { argcount: 1 }),
    'include': tags.simple_tag(nodes.IncludeNode, { argcount: 1 }),
    ...
    'templatetag': tags.simple_tag(nodes.TemplateTagNode, { argcount: 1 }),
    'spaceless': function (parser, token) {
        var parts = tags.get_args_from_token(token, { argcount: 0 });
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.SpacelessNode(node_list);
    },
    'widthratio': tags.simple_tag(nodes.WithRatioNode, { argcount: 3 }),
    'regroup': tags.simple_tag(nodes.RegroupNode, { argcount: 5, mustbe: { 2: 'by', 4: 'as' }, exclude: [2, 4] }),
