...
var utils_tags = require('../utils/tags');
...
var nodes = exports.nodes = {
    ...
    LoadNode: function (path, package) {
        return function (context, callback) {
            common.mixin(context.filters, package.filters);
            callback(false, '');
        }
    },
    ...
    UrlNode: function (url_name, replacements, item_name) {
        ...
        return function (context, callback) {
			// TODO: figure out how to not use drty for this
			var patterns = drty.urls.getPatterns();
			var url = patterns.reverse.apply(patterns, [context.get(url_name)].concat(
				replacements.map(function (x) { return context.get(x); })));
...
var tags = exports.tags = {
    ...
    'for': function (parser, token) {
        ...
        var parts = utils_tags.get_args_from_token(token, { exclude: 2, mustbe: { 2: 'in', 4: 'reversed'} });
    ...
    'ifchanged': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token);
    ...
    'ifequal': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token, { argcount: 2 });
    ...
    'autoescape': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token, { argcount: 1, mustbe: { 1: ['on', 'off'] }});
    ...
    'block': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token, { argcount: 1 });
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.BlockNode(node_list, parts[0]);
    },

    'extends': utils_tags.simple_tag(nodes.ExtendsNode, { argcount: 1 }),

    'firstof': utils_tags.simple_tag(nodes.FirstOfNode),

    'with': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token, { argcount: 3, exclude: 2, mustbe: { 2: 'as' }});
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.WithNode(node_list, parts[0], parts[1], parts[2]);
    },
    
    'now': utils_tags.simple_tag(nodes.NowNode, { argcount: 1 }),
    'include': utils_tags.simple_tag(nodes.IncludeNode, { argcount: 1 }),
    'load': function (parser, token) {
        ...
        common.mixin(parser.tags, package.tags);

        return nodes.LoadNode(name, package);
    },
    'templatetag': utils_tags.simple_tag(nodes.TemplateTagNode, { argcount: 1 }),
    'spaceless': function (parser, token) {
        var parts = utils_tags.get_args_from_token(token, { argcount: 0 });
        var node_list = parser.parse('end' + token.type);
        parser.delete_first_token();
        return nodes.SpacelessNode(node_list);
    },
    'widthratio': utils_tags.simple_tag(nodes.WithRatioNode, { argcount: 3 }),
    'regroup': utils_tags.simple_tag(nodes.RegroupNode, { argcount: 5, mustbe: { 2: 'by', 4: 'as' }, exclude: [2, 4] }),
