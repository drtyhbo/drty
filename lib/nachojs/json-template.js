...
function _Ellipsize(value, unused_context, args) {
	value = String(value);
...
function _DoCall(statement, context, callback) {
  var lastPeriod = statement.name.lastIndexOf('.');
  if (lastPeriod != -1) {
    var callContext = context.get(statement.name.substr(0, lastPeriod)),
        fnc = callContext[statement.name.substr(lastPeriod + 1)];
  } else {
	  var callContext = this,
	      fnc = context.get(statement.name);
  }
  if (!fnc) {
    callback('');
  } else {
	var args = statement.arguments.slice();
	for (var i = 0; i < args.length; i++) {
		args[i] = context.get(args[i]) || args[i];
	}
    callback(fnc.apply(callContext, args));
  }
}
...
function _Compile(template_str, options) {
  ...
  var default_formatters = PrefixRegistry([
      ...
      {name: 'ellipsize', func: _Ellipsize}
      ]);
  ...
  function GetFormatter(format_str) {
	var pieces = format_str.split(':');
    var pair = all_formatters.lookup(pieces[0]);
    if (!pair[0]) {
      throw {
        name: 'BadFormatter',
  ...
  while (true) {
    ...
    if (token.charAt(0) == '.') {  // Keyword
      ...
      var call_match = token.match(_CALL_RE);
      if (call_match) {
        var pred_str = call_match[1].replace(/\s+/g, ' ');
        var pieces = pred_str.split(' ');
        current_block.Append([_DoCall, {name: pieces[0], arguments: pieces.slice(1)}]);
        continue;
      }
...
exports.CallableRegistry = CallableRegistry;
exports.ChainedRegistry = ChainedRegistry;
