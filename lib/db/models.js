...
exports.unserialize = function(entries, cb) {
	var modelsByTable = {};
	...
	drty.utils.eachChained(entries, function(name, entry, next) {
		...
		Model.unserialize(entry, function(err) {
			if (err) { throw err; }
			next();
		});
	});
...
var Model = exports.Model = drty.Class.extend({
	...
	save: function(options, callback) {
		...
		drty.utils.eachChained(this.getColumns(), function(name, column, next) {
			...
			if ((isNew && !options.forceUpdate)
					|| options.forceInsert) {
				conn.insert(this, callback);
			} else {
