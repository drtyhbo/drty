...
var Field = drty.Class.extend({
	...
	getInitialValue: function() {
		if (!('default' in this._options)) { return null; }

		return typeof(this._options.default) == 'function'
			? this._options.default()
			: this._options.default;
	},
	prepValue: function(value, callback) { callback(null, value); },
...
exports.DateTimeField = Field.extend({
	...
	prepValue: function(value, callback) {
		if (!value) {
			callback(null, value);
		} else {
			if (this.isAutoNow()) { value = new Date(); }
			callback(null, new Date(value.getFullYear(), value.getMonth(), value.getDate(),
...
exports.DateField = Field.extend({
	...
	prepValue: function(value, callback) {
		if (!value) {
			...
			if (this.isAutoNow()) { value = new Date(); }
			callback(null, new Date(value.getFullYear(), value.getMonth(),
				value.getDate()));
		}
	}
});
...
exports.FileField = exports.CharField.extend({
	...
	prepValue: function(value, callback) {
		...
		var mediaRoot = drty.conf.settings.MEDIA_ROOT;
		if (!mediaRoot) {
			throw new Error('FileField requires that MEDIA_ROOT be in the settings');
		}

		var fs = require('fs'),
			destRelPath = require('path').join(this._options.uploadTo, value.name),
			destPath = require('path').join(mediaRoot, destRelPath),
			readStream = fs.createReadStream(value.path),
			writeStream = fs.createWriteStream(destPath);

		require('sys').pump(readStream, writeStream, function(err) {
			callback(err, destRelPath);
		});
	}
});
