...
module.exports = testCase({
	'insert': function(test) {
		...
		test.equal(new statements.Insert(model).getSQL(),
			"INSERT INTO `mymodel` (integer_field, char_field, text_field, boolean_field, date_time_field, foreign_key_field_id) VALUES(100, \"what\'s up G?\", NULL, 1, \"2011-01-11 20:49:51\", NULL)");
	...
	'update': function(test) {
		...
		test.equal(new statements.Update(model).getSQL(),
			"UPDATE `mymodel` SET integer_field=100, char_field=\"what's up G?\", text_field=NULL, boolean_field=1, date_time_field=\"2011-01-11 20:49:51\", foreign_key_field_id=NULL WHERE id=20");
	...
	'create': function(test) {
		test.equal(new statements.Create(MyModel).getSQL(),
			'CREATE TABLE IF NOT EXISTS `mymodel` (integer_field integer NOT NULL, char_field varchar(32) NOT NULL, text_field text, boolean_field tinyint UNSIGNED NOT NULL, date_time_field datetime, foreign_key_field_id integer UNSIGNED, id integer UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY(id))');
		test.done();
	}
});