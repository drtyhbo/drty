...
var MyModel = models.Model.extend({
	...
	SmallIntegerField: new models.SmallIntegerField(),
	...
	NullBooleanField: new models.NullBooleanField(),
...
module.exports = testCase({
	'meta': function(test) {
		...
		test.equal(meta.getSqlName(), 'small_integer_field');
		test.equal(meta.getSqlType(), 'smallint');
		test.equal(meta.getQualifiedName(), 'mymodel.small_integer_field');
		test.equal(meta.getSelectName(), 'mymodel__small_integer_field');
		test.equal(meta.getColumnDef(), 'small_integer_field smallint NOT NULL');
		test.equal(meta.toSQL(100), 100);
		test.equal(meta.toJS(100), 100);
		...
		test.equal(meta.getColumnDef(), 'null_boolean_field tinyint UNSIGNED');
		test.strictEqual(meta.toSQL(true), 1);
		test.strictEqual(meta.toSQL(false), 0);
		test.strictEqual(meta.toJS(1), true);
		test.strictEqual(meta.toJS(0), false);
