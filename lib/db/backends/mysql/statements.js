...
exports.Select = drty.Class.extend({
	...
	getSelectParts: function(Model, SELECT, FROM, WHERE, selectRelated) {
		...
		if (selectRelated) {
			...
			for (var name in foreignKeys) {
				...
				this.getSelectParts(foreignKey.otherModel, SELECT, FROM, WHERE, selectRelated);
	...
	getSQL: function() {
		...
		this.getSelectParts(Model, SELECT, FROM, WHERE, qs.selectRelated)
